...
# Optional: SonarQube token for code analysis
...
| `POST` | `/api/v1/auth/login` | No | Authenticate credentials and receive a JWT Bearer token |
| `GET` | `/api/v1/auth/me` | Bearer Token | Retrieve profile information for the authenticated user |
| `POST` | `/api/v1/auth/logout` | No / Token | Clear security context and invalidate session locally |
